...
func (tc *TCache) rebuild(
	...
	// expect and return the same types
	if reflect.TypeOf(result) != reflect.TypeOf(in) {
		return fmt.Errorf(
			"data type mismatch, expected %s, got %s",
			reflect.TypeOf(in),
...
func (tc *TCache) lockRebuild(mkey string) (bool, error) {
	// try to take lock on rebuilding cache
	// in case of everyone goes rebuilding it
	// params should be tuned
	lockKey := "lock_" + mkey
	lockAccuired := false
	for i := 0; i < 4; i++ {
		// Add will add a record only if it doesn't exist yet
